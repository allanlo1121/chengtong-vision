...
export default function Pie() {
  ...
  useEffect(() => {
    ...
    //绘制推进主界面
    for (let i = 0; i < num; i++) {
      // const angleStart = -Math.PI / 2 + ((i * 2 - 1) / num) * Math.PI; // 起始角度
      // const angleEnd = -Math.PI / 2 + ((i * 2 + 1) / num) * Math.PI; // 结束角度
      // 计算每个扇形的角度范围
      ...
      // 颜色根据 i 生成 HSL（彩虹渐变）
      ctx.fillStyle = `hsl(${(i / num) * 360}, 80%, 60%)`;
    ...
    ctx.moveTo(0, 0); // 从中心开始
    ctx.strokeRect(-80, -100, 160, 200); // 画圆弧
    ...
    thrustData.forEach((data, index) => {
      ...
      ctx.strokeStyle = "red";
      ctx.strokeRect(cx, cy, 160, rectHeight);

      ctx.translate(cx + 80, cy + rectHeight / 2); // 平移坐标系到矩形中心
