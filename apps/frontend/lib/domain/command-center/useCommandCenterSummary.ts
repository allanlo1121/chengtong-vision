...
import { fetchCommandCenterSummary } from "@/lib/domain/command-center/client.service";
...
export function useCommandCenterSummary(initialData: CommandCenterSummary) {
  const supabase = React.useMemo(() => createClient(), []);

  const [data, setData] = React.useState<CommandCenterSummary | null>(initialData);
  ...
  const refetch = React.useCallback(async () => {
    try {
      ...
      const next = await fetchCommandCenterSummary();
      setData(next);
    } catch (err) {
      setError(err instanceof Error ? err.message : "获取指挥中心数据失败");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
