...
export async function fetchFilteredSubprojects(
  ...
  try {
    ...
    const { data: subprojects, error } = await filterQuery;
    ...
    if (subprojects && subprojects.length > 0) {
      ...
      const result: ISubproject[] = camelData.map((project) => ({
        id: project.id,
        name: project.name,
        shortName: project.shortName,
        projectShortName: project.projectShortName,
        regionName: project.regionName,
        tbmName: project.tbmName,
        ringStart: project.ringStart,
        ringEnd: project.ringEnd,
        opNumStart: project.opNumStart,
        opNumEnd: project.opNumEnd,
        startDate: project.planStartDate,
        endDate: project.planEndDate,
