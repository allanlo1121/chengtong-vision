...
// 递归转换对象或数组的所有 key，从 snake_case ➜ camelCase
export function convertKeysToCamelCase(input: any): any {
  if (Array.isArray(input)) {
    return input.map(convertKeysToCamelCase);
  } else if (input !== null && typeof input === 'object') {
    return Object.keys(input).reduce((result: any, key) => {
      const camelKey = toCamelCase(key);
      result[camelKey] = convertKeysToCamelCase(input[key]);
      return result;
    }, {});
  }
  return input;
