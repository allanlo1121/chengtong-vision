...
// import { ConnectivityRepo } from "@core/alarm/reposistory/connectivityRepo.js"
...
async function resolveAffectedTbmIds(change: any): Promise<string[]> {
  ...
  if (table === "tbm_assignments") {
    const tbmId = change.new?.tbm_id || change.old?.tbm_id;
  ...
  if (table === "employees") {
    const employeeId = change.new?.id || change.old?.id;
...
export function startRealtimeListeners() {
  ...
  const tables = ["tbm_assignments", "tunnels", "project_leader_history", "projects", "employees"];
   ...
   * ===================================================== */
  const thresholdTables = ["tbm_threshold_rules", "tbm_threshold_rules_per_tbm"];
  ...
  for (const table of thresholdTables) {
    supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table }, async () => {
        ...
        await MetadataRegistry.thresholdRules.refreshAll();
      })
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
  ...
  const parameterTables = ["tbm_runtime_parameters", "tbm_subsystems"];
  ...
  for (const table of parameterTables) {
    supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table }, async () => {
        ...
        await MetadataRegistry.parameterMetas.initialize();
        await MetadataRegistry.parameterGroups.initialize();
      })
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
...
export async function handleChange(payload: any) {
  try {
    ...
    for (const tbmId in tbmIds) {
      await MetadataRegistry.tbmContexts.refreshTbm(tbmId);
    }
    ...
    if (table === "tbm_assignments") {
      ...
      if (status === "WORKING") {
        // await ConnectivityRepo.upsert({
        //     tbm_id: tbmId,
        //     param_code: 'n010000001',
        //     status: false,
        //     start_at: new Date().toISOString()
        // });
        // await ConnectivityRepo.upsert({
        //     tbm_id: tbmId,
        //     param_code: 'n010000002',
        //     status: false,
        //     start_at: new Date().toISOString()
        // });
      } else {
        // await ConnectivityRepo.delete({
        //     tbm_id: tbmId,
        //     param_code: 'n010000001'
        // })
        // await ConnectivityRepo.delete({
...
async function clearActiveParameterState(tbmId: string) {
  const { error } = await supabase.from("tbm_active_static_events").delete().eq("tbm_id", tbmId);

  if (error) {
    logger.error("❌ 删除 tbm_active_static_events 出错", error);
  } else {
    logger.info(`🧹 清理 tbm_active_static_events 完成 → TBM=${tbmId}`);
  }
}
